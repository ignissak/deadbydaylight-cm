    ...
    private fun getLines(gamePlayer: GamePlayer): HashMap<String, Int> {
        ...
        when (DeadByDaylight.gameManager.gameState) {
            ...
            GameState.INGAME -> {
                ...
                hashMap["§6"] = 6
                hashMap["§fSurvivoři: §c${PlayerManager.survivorTeam.entries.stream().filter { it.getSurvivor()?.playerState != SurvivalState.SPECTATING }.count().toInt()}/${DeadByDaylight.gameManager.startingPlayers - 1}"] = 5
